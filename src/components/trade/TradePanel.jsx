...
const TradePanel = ({ stockCode, initialPrice, initialHoka, resetTab }) => {
  ...
  const fetchOrderHistory = async () => {
    try {
      // 현금 주문 내역 가져오기
      const cashResponse = await fetch('http://localhost:5002/api/order_history');
      const cashData = await cashResponse.json();
      if (cashResponse.ok) {
        const cashFilled = cashData.filter((order) => parseInt(order.tot_ccld_qty) === parseInt(order.ord_qty));
        const cashUnfilled = cashData.filter((order) => parseInt(order.tot_ccld_qty) < parseInt(order.ord_qty));
      ...
      const creditResponse = await fetch('http://localhost:8080/api/transaction_history');
      const creditData = await creditResponse.json();
      if (creditResponse.ok) {
        setCreditFilledOrders(creditData);
      } else {
        console.error('Failed to fetch credit transaction history:', creditData.error);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      try {
        const response = await fetch(`http://localhost:5002/api/balance?code=${stockCode}`);
  ...
  const placeOrder = async () => {
    try {
      ...
      if (orderDetails.trade_type === '유통대주신규' || orderDetails.trade_type === '유통대주상환') {
  ...
  const increasePrice = () => {
    setPrice((prevPrice) => {
      ...
      const numericHoka = parseInt(initialHoka.toString().replace(/,/g, ''), 10);
  ...
  const decreasePrice = () => {
    setPrice((prevPrice) => {
      const numericPrice = parseInt(prevPrice.toString().replace(/,/g, ''), 10);
      const numericHoka = parseInt(initialHoka.toString().replace(/,/g, ''), 10);
      const updatedPrice = numericPrice > numericHoka ? numericPrice - numericHoka : 0;
    ...
    <div className="trade-panel-order-section">
      ...
      >
        <div className="trade-panel-order-type">
          ...
          <button
            className={`trade-panel-type-button ${tradeType === '현금' ? 'active' : ''}`}
          ...
          <div className="trade-panel-type-button-wrapper">
            ...
            {showTradeOptions && (
              <div className="trade-options-dropdown" style={{ height: 'auto' }}>
                <button className="trade-option" onClick={() => handleTradeOptionClick('자기융자신규')}>
                  자기융자신규
                </button>
                <button className="trade-option" onClick={() => handleTradeOptionClick('유통융자신규')}>
                  유통융자신규
                </button>
                <button className="trade-option" onClick={() => handleTradeOptionClick('유통대주상환')}>
                  유통대주상환
                </button>
              </div>
            )}
        ...
        <div className="trade-panel-order-type">
          ...
          <div className="trade-panel-type-button-container">
            <div className="trade-panel-type-button-wrapper">
              <button
                className={`trade-panel-type-button ${orderType.includes('지정가') ? 'active' : ''}`}
              ...
              {showOrderOptions && (
                <div className="order-options-dropdown">
                  <button className="order-option" onClick={() => handleOrderOptionClick('지정가')}>
                    지정가
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('조건부지정가')}>
                    조건부지정가
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('최유리지정가')}>
                    최유리지정가
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('최우선지정가')}>
                    최우선지정가
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('시간외단일가')}>
                    시간외단일가
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('지정가(IOC)')}>
                    지정가(IOC)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('시장가(IOC)')}>
                    시장가(IOC)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('최유리(IOC)')}>
                    최유리(IOC)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('지정가(FOK)')}>
                    지정가(FOK)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('시장가(FOK)')}>
                    시장가(FOK)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('최유리(FOK)')}>
                    최유리(FOK)
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('장전시간외')}>
                    장전시간외
                  </button>
                  <button className="order-option" onClick={() => handleOrderOptionClick('장후시간외')}>
                    장후시간외
                  </button>
                </div>
              )}
            ...
            <button
              className={`trade-panel-type-button ${orderType === '시장가' ? 'active' : ''}`}
        ...
        <div className="trade-panel-input-group">
          ...
          <div className="trade-panel-input">
            <button className="trade-panel-adjust-button" onClick={decreasePrice} disabled={orderType === '시장가'}>
              –
            </button>
            <input
              type="text"
            ...
            />
            <button className="trade-panel-adjust-button" onClick={increasePrice} disabled={orderType === '시장가'}>
              +
            </button>
          </div>
        </div>
        ...
        <div className="trade-panel-input-group">
          <label>주문 수량</label>
          <div className="trade-panel-input">
            <button className="trade-panel-adjust-button" onClick={decreaseQuantity}>
              –
            </button>
            <input type="text" className="trade-panel-input-field" value={quantity} onChange={handleQuantityChange} />
            <button className="trade-panel-adjust-button" onClick={increaseQuantity}>
              +
            </button>
          </div>
        </div>

        <div className="trade-panel-input-group" style={{ marginBottom: '30px' }}>
      ...
      >
        <div className="trade-panel-order-type">
          ...
          <div className="trade-panel-type-button-wrapper">
            ...
            {showTradeOptions && (
              <div className="trade-options-dropdown" style={{ height: 'auto' }}>
                <button className="trade-option" onClick={() => handleTradeOptionClick('자기융자상환')}>
                  자기융자상환
                </button>
                <button className="trade-option" onClick={() => handleTradeOptionClick('유통융자상환')}>
                  유통융자상환
                </button>
                <button className="trade-option" onClick={() => handleTradeOptionClick('유통대주신규')}>
                  유통대주신규
                </button>
                <button className="trade-option" onClick={() => handleTradeOptionClick('담보대출상환')}>
                  담보대출상환
                </button>
              </div>
            )}
    ...
    <div className="trade-panel-container">
      <div className="trade-panel-tabs">
        ...
        <div
          ...
          className={`trade-panel-tab ${activeTab === '정정/취소' ? 'active' : ''}`}
        ...
        <div
          ...
          className={`trade-panel-tab ${activeTab === '체결/예약' ? 'active' : ''}`}
      ...
      >
        <div style={modalStyle}>
          ...
          <div id="uptick-rule-modal-description">
            <p>
              업틱룰은 투자자가 공매도를 할 때 가장 최근 체결 가격보다 높은 가격으로만 호가를 제출할 수 있도록 하는
              규정입니다.
            </p>
          </div>
