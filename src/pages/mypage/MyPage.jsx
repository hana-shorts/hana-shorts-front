...
const MyPage = () => {
  ...
  const [preliminaryEducationCompleted, setPreliminaryEducationCompleted] = useState(false);
  ...
  // 사전교육 입력 상태
  const [educationCompletionNumber, setEducationCompletionNumber] = useState('');
  ...
  const [mockTradingCompletionDate, setMockTradingCompletionDate] = useState('');
  const [mockTradingCompletionTime, setMockTradingCompletionTime] = useState('');
  const [mockTradingCustomerID, setMockTradingCustomerID] = useState('');
  const [mockTradingAuthenticationKey, setMockTradingAuthenticationKey] = useState('');
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // 완료 상태 설정
          setPreliminaryEducationCompleted(userData.preliminaryEducationCompleted === 'YES');
          ...
          if (creditResponse.data && creditResponse.data.length > 0) {
            ...
            const currentPricePromises = creditStockCodes.map((code) =>
              axios
                ...
                .catch((err) => {
                  console.error(`Failed to fetch current price (${code}):`, err);
            ...
            const creditHoldingsData = creditResponse.data.map((order) => {
              ...
              const profitRate = purchaseAmount !== 0 ? (profitOrLoss / purchaseAmount) * 100 : 0;
  ...
  useEffect(() => {
    ...
    if (balanceInfo && holdings.length >= 0 && creditHoldings.length >= 0) {
      ...
      const totalCashProfitOrLoss = holdings.reduce(
        (sum, holding) => sum + parseFloat(holding.evlu_pfls_amt.replace(/,/g, '')),
        0
      );

      // 신용 보유 내역 총합
      const totalCreditPurchaseAmount = creditHoldings.reduce((sum, holding) => sum + holding.total_price, 0);

      const totalCreditEvaluationAmount = creditHoldings.reduce((sum, holding) => sum + holding.total_current_price, 0);

      const totalCreditProfitOrLoss = creditHoldings.reduce((sum, holding) => sum + holding.profit_or_loss, 0);

      // 총합 계산
      const totalPurchase = totalCashPurchaseAmount + totalCreditPurchaseAmount;
      const totalEvaluation = totalCashEvaluationAmount + totalCreditEvaluationAmount;
      const totalProfitLoss = totalCashProfitOrLoss + totalCreditProfitOrLoss;
      const totalProfitRateCalc = totalPurchase !== 0 ? (totalProfitLoss / totalPurchase) * 100 : 0;
  ...
  const handleEducationRegistration = async () => {
    try {
      const response = await axios.post('/api/user/registerPreliminaryEducation', {
    ...
    <div className="mypage-page fade-in-minus-y">
      ...
      <div className="mypage-info-registration-section">
        ...
        <div className="mypage-registration-item">
          <button
            className={`mypage-registration-button ${preliminaryEducationCompleted ? 'completed' : 'active'}`}
            onClick={() => {
          ...
          >
            <Box sx={modalStyle}>
              ...
              <div className="mypage-modal-information">
                <ul>
                  <li>대주 사전교육 및 모의거래 이수정보를 등록하는 화면입니다.</li>
                  <li>
                    금융투자협회 사전교육(
                    <a href="https://www.kifin.or.kr" target="_blank" rel="noopener noreferrer">
                      https://www.kifin.or.kr
                    </a>
                    ) 및 한국거래소 (
                    <a href="http://strn.krx.co.kr" target="_blank" rel="noopener noreferrer">
                      http://strn.krx.co.kr
                    </a>
                    ) 또는 각 증권사 모의거래 시스템을 이용한 모의거래 이수완료 후, 해당 이수정보를 화면에 등록하시기
                    바랍니다.
                  </li>
                  <li>
                    대주거래는 신용약정(대주거래위험고지 포함) 및 사전투자경험을 충족하셔야 대주거래가 가능합니다.
                  </li>
                  <li>온라인을 통해 사전교육/모의거래를 등록한 경우, 영업점 승인이 완료되어야 거래 가능합니다.</li>
                  <li className="mypage-red-text">
                    승인절차는 영업점 업무 마감시점에 일괄 처리되오니, 등록 후 바로 거래를 원하시는 고객은
                    계좌개설영업점 또는 고객센터(1588-3111)로 연락하시기 바랍니다.
                  </li>
                </ul>
              </div>

              {educationErrorMessage && <div className="mypage-error-message">{educationErrorMessage}</div>}
              <button className="register-button" onClick={handleEducationRegistration} style={{ marginTop: '20px' }}>
                교육 이수 등록
              </button>
            </Box>
          </Modal>
          {preliminaryEducationCompleted ? (
            <div className="mypage-completion-message">사전교육 이수가 완료되었습니다.</div>
          ) : (
            <div className="mypage-warning-message">사전교육을 아직 등록하지 않았습니다.</div>
          )}
        </div>
        ...
        <div className="mypage-registration-item">
          <button
            className={`mypage-registration-button ${mockTradingCompleted ? 'completed' : 'active'}`}
            onClick={() => {
          ...
          >
            <Box sx={modalStyle}>
              ...
              <div className="mypage-modal-description">
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) => setMockTradingAuthenticationKey(e.target.value)}
                    className="mypage-input"
                  />
                </label>
              </div>
              ...
              {mockTradingErrorMessage && <div className="mypage-error-message">{mockTradingErrorMessage}</div>}
              <button className="register-button" onClick={handleMockTradingRegistration} style={{ marginTop: '20px' }}>
                교육 이수 등록
              </button>
            </Box>
          </Modal>
          {mockTradingCompleted ? (
            <div className="mypage-completion-message">모의거래 이수가 완료되었습니다.</div>
          ) : (
            <div className="mypage-warning-message">모의거래를 아직 등록하지 않았습니다.</div>
          )}
        </div>
      </div>
        ...
        <div className="mypage-balance-summary">
          <div className="mypage-profit-loss">
            ...
            <div>
              ...
              <span className="percentage">({totalProfitRate.toFixed(2)}%)</span>
            </div>
          </div>
                  ...
                  <tbody>
                    {holdings.map((holding, index) => (
                      <tr key={index}>
                        ...
                        <td>{formatNumber(Number(parseFloat(holding.pchs_avg_pric).toFixed(0)))}</td>
