...
const sortedIndicators = indicators.sort((a, b) => a.displayName.localeCompare(b.displayName, 'ko'));
...
const recommendedCombinations = [
  {
    id: 1,
    name: '추세 추종 조합',
    indicators: ['Moving Average (MA)', 'MACD', 'Average Directional Index (ADX)'],
    reason: '추세의 방향과 강도를 파악하여 안정적인 매매 신호를 제공합니다.',
  },
  {
    id: 2,
    name: '과매수/과매도 조합',
    indicators: ['Relative Strength Index (RSI)', 'Stochastic Oscillator', 'Commodity Channel Index (CCI)'],
...
const ResearchBox = () => {
  ...
  const combinedSectors = [...new Set([...selectedKospiSectors, ...selectedKosdaqSectors])];
  ...
  const handleMarketChange = (market) => {
    setSelectedMarkets((prevSelected) =>
      prevSelected.includes(market) ? prevSelected.filter((item) => item !== market) : [...prevSelected, market]
    );
  };

  const handleKospiSectorChange = (sector) => {
    setSelectedKospiSectors((prevSelected) =>
      prevSelected.includes(sector) ? prevSelected.filter((item) => item !== sector) : [...prevSelected, sector]
    );
  };
  ...
  const handleNext = () => {
    if (stepState === 1) {
      ...
      if (selectedKospiSectors.length === 0 && selectedKosdaqSectors.length === 0) {
        setError('업종을 선택해주세요.');
        return;
      }
      setStepState(3);
    ...
    switch (stepState) {
      case 1:
        ...
        if (kospiData.length === 0) {
          ...
          const percentChangeFormatted = priceChange > 0 ? `+${percentChange}%` : `${percentChange}%`;
          const changeColor = priceChange > 0 ? '#dc3545' : '#007bff';

          kospiContent = (
            <Card
              onClick={() => handleMarketChange('KOSPI')}
              className={`researchbox-market-card ${selectedMarkets.includes('KOSPI') ? 'selected' : ''}`}
            >
              <CardContent className="researchbox-market-card-content">
                <div className="researchbox-market-name" style={{ color: changeColor }}>
                  KOSPI
                </div>
                <div className="researchbox-market-price">{latestPrice.toLocaleString()}</div>
                <div className="researchbox-market-change" style={{ color: changeColor }}>
                ...
                <div className="researchbox-market-card-chart">
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={kospiData}>
                      <defs>
                        <linearGradient id={`colorLine-KOSPI`} x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={changeColor} stopOpacity={0} />
                          <stop offset="100%" stopColor={changeColor} stopOpacity={0.5} />
                        </linearGradient>
                      </defs>
        ...
        if (kosdaqData.length === 0) {
          ...
          kosdaqContent = (
            <Card
              ...
              className={`researchbox-market-card ${selectedMarkets.includes('KOSDAQ') ? 'selected' : ''}`}
            >
              <CardContent className="researchbox-market-card-content">
                <div className="researchbox-market-name" style={{ color: changeColor }}>
                  KOSDAQ
                ...
                <div className="researchbox-market-card-chart">
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={kosdaqData}>
                      <defs>
                        <linearGradient id={`colorLine-KOSDAQ`} x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={changeColor} stopOpacity={0} />
                          <stop offset="100%" stopColor={changeColor} stopOpacity={0.5} />
                        </linearGradient>
                      </defs>
        ...
        return (
          <>
            <div className="researchbox-header">추천받고자 하는 시장을 선택해주세요</div>
            ...
            <div className="researchbox-button-container">
              ...
              <button onClick={handleNext} className="researchbox-button" style={{ marginRight: '200px' }}>
                다음으로
              </button>
            </div>
          </>
      ...
      case 2:
        ...
        return (
          <>
            <div className="researchbox-header">관심 있는 업종은 무엇인가요?</div>
                    ...
                    {kospiSectors.map((sector, index) => (
                      <button
                        ...
                        className={`researchbox-sector-button ${
                          selectedKospiSectors.includes(sector) ? 'selected' : ''
                        }`}
                  ...
                  <div className="researchbox-sectors">
                    {kosdaqSectors.map((sector, index) => (
                      <button
                        key={`kosdaq-${index}`}
                        className={`researchbox-sector-button ${
                          selectedKosdaqSectors.includes(sector) ? 'selected' : ''
                        }`}
            ...
            <div className="researchbox-button-container">
              <button onClick={handlePrevious} className="researchbox-button" style={{ marginLeft: '200px' }}>
                이전으로
              </button>
              <button onClick={handleNext} className="researchbox-button" style={{ marginRight: '200px' }}>
                다음으로
      ...
      case 3:
        ...
        return (
          <>
            <div className="researchbox-header">분석에 활용할 지표를 선택해 주세요</div>
            <div className="researchbox-recommendation-container">
              <div className="researchbox-recommendations">
                {recommendedCombinations.map((combo) => (
                  <CustomTooltip key={combo.id} title={combo.reason} arrow placement="top">
                    <button
            ...
            <div className="researchbox-sectors-container" style={{ height: '400px' }}>
              <div className="researchbox-sector-section">
                <div className="researchbox-sector-header">지표 목록</div>
                <div className="researchbox-sectors">
                  {sortedIndicators.map((indicator, index) => (
                    <CustomTooltip key={index} title={indicator.description} arrow placement="top">
                      <button
                        className={`researchbox-sector-button ${
                          selectedIndicators.includes(indicator.name) ? 'selected' : ''
                        }`}
      ...
      >
        <div className="researchbox-modal">
          ...
          <div className="researchbox-modal-buttons">
            <Button variant="contained" color="primary" onClick={handleAIModalConfirm}>
              O
            </Button>
            <Button variant="contained" color="secondary" onClick={handleAIModalClose}>
              X
            </Button>
          </div>
        </div>
